...
class FOnlineAsyncEventSteamInvitedToGame : public FOnlineAsyncEvent<FOnlineSubsystemSteam>
{
private:
	...
	/** Hidden on purpose */
	FOnlineAsyncEventSteamInvitedToGame() :
...
public:
	...
	FOnlineAsyncEventSteamInvitedToGame(FOnlineSubsystemSteam* InSubsystem, const FUniqueNetIdSteam& InFriendId, const FString& InInviteParam) :
		FOnlineAsyncEvent(InSubsystem),
		FriendId(InFriendId),
		InviteParam(InInviteParam)
	{
	}
	
	virtual ~FOnlineAsyncEventSteamInvitedToGame() 
	{
	}
	...
	{
		return FString::Printf(TEXT("FOnlineAsyncEventSteamInvitedToGame %s,%s"), 
	...
	{
		Subsystem->TriggerOnSteamInvitedToGameDelegates(InviteParam);
	}
};
...
{
	FOnlineAsyncEventSteamInvitedToGame* NewEvent = 
		new FOnlineAsyncEventSteamInvitedToGame(SteamSubsystem, FUniqueNetIdSteam(CallbackData->m_steamIDFriend), UTF8_TO_TCHAR(CallbackData->m_rgchConnect));
	UE_LOG_ONLINE(Verbose, TEXT("%s"), *NewEvent->ToString());
	UE_LOG(LogTemp,Warning,TEXT("OnInviteAccepted %s"),*NewEvent->ToString());
...
class FOnlineAsyncEventSteamMicroTxnAuthorizationResponse : public FOnlineAsyncEvent<FOnlineSubsystemSteam>
{
	FOnlineAsyncEventSteamMicroTxnAuthorizationResponse() :
		FOnlineAsyncEvent(NULL)
	{
	}

	uint32 AppID;			// AppID for this microtransaction
	uint64 OrderID;			// OrderID provided for the microtransaction
	uint8 bAuthorized;		// if user authorized transaction

public:

	FOnlineAsyncEventSteamMicroTxnAuthorizationResponse(FOnlineSubsystemSteam* InSubsystem, uint32 InAppID, uint64 InOrderID, uint8 InbAuthorized) :
		FOnlineAsyncEvent(InSubsystem),
		AppID(InAppID),
		OrderID(InOrderID),
		bAuthorized(InbAuthorized)
	{
	}
	...
	virtual FString ToString() const override
	{
		return FString::Printf(TEXT("FOnlineAsyncEventSteamMicroTxnAuthorizationResponse %d,%llu,%d"), AppID, OrderID, bAuthorized);
	}
	...
	virtual void Finalize() override
	{
		if( FCString::Atoi(*Subsystem->GetAppId()) != AppID)
		{
			UE_LOG_ONLINE(Error,TEXT("FOnlineAsyncEventSteamMicroTxnAuthorizationResponse AppId Wrong %s,%d,%llu,%s"),*Subsystem->GetAppId(),AppID,OrderID,bAuthorized?TEXT("true"):TEXT("false"))
		}
		Subsystem->TriggerOnSteamMicroTxnAuthorizationResponseDelegates(OrderID,!!bAuthorized);
	}
};
void FOnlineAsyncTaskManagerSteam::OnMicroTxnAuthorizationResponse(MicroTxnAuthorizationResponse_t* CallbackData)
{
	UE_LOG_ONLINE(Verbose, TEXT("OnMicroTxnAuthorizationResponse %d,%llu,%d"),CallbackData->m_unAppID, CallbackData->m_ulOrderID,(int)CallbackData->m_bAuthorized);
	FOnlineAsyncEventSteamMicroTxnAuthorizationResponse* NewEvent = new FOnlineAsyncEventSteamMicroTxnAuthorizationResponse(SteamSubsystem, CallbackData->m_unAppID, CallbackData->m_ulOrderID, CallbackData->m_bAuthorized);
	AddToOutQueue(NewEvent);
}