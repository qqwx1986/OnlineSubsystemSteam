...
UTexture2D* FOnlineFriendSteam::GetUserImage() const
{
	UTexture2D* Avatar = UTexture2D::CreateTransient(uAvatarWidth, uAvatarHeight, PF_B8G8R8A8);

	uint8* MipData = (uint8*)Avatar->PlatformData->Mips[0].BulkData.Lock(LOCK_READ_WRITE);
	FMemory::Memcpy(MipData, (void*)pAvatarRGBA, uAvatarHeight * uAvatarWidth * 4);
...
{
	// SteamFriendsPtr->InviteUserToGame()
	bool Result = false;
	if (LocalUserNum < MAX_LOCAL_PLAYERS &&
		SteamUserPtr != NULL &&
		SteamUserPtr->BLoggedOn() &&
		...
		if (FriendsList != NULL)
		{
			for (int32 FriendIdx=0; FriendIdx < FriendsList->Friends.Num(); FriendIdx++)
			{
				if (*FriendsList->Friends[FriendIdx]->GetUserId() == FriendId)
				{
					...
					Result = SteamFriendsPtr->InviteUserToGame(SteamPlayerId,TCHAR_TO_ANSI(*ListName));
					break;
				}
			}
		}
	...
	Delegate.ExecuteIfBound(LocalUserNum, Result, FriendId, ListName, FString());
	return Result;
}
...
void FOnlineAsyncTaskSteamReadFriendsList::Finalize()
{
	...
	for (int32 Index = 0; Index < NumFriends; Index++)
	{
		...
		int nLevel = SteamFriendsPtr->GetFriendSteamLevel(SteamPlayerId);
		FString JoinablePresenceString = UTF8_TO_TCHAR(SteamFriendsPtr->GetFriendRichPresence(SteamPlayerId, "Joinable"));
		uint32 uAvatarWidth(0), uAvatarHeight(0);
		uint8 *pAvatarRGBA = nullptr;
		int iImage =  SteamFriendsPtr->GetLargeFriendAvatar(SteamPlayerId);
		if(iImage>0)
		{
			if(SteamUtils()->GetImageSize( iImage, &uAvatarWidth, &uAvatarHeight ) && uAvatarWidth>0 && uAvatarHeight>0 )
			{
				const int AvatarLen = uAvatarWidth * uAvatarHeight * 4;
				pAvatarRGBA = new uint8[ AvatarLen];
				memset(pAvatarRGBA,0,AvatarLen);
				SteamUtils()->GetImageRGBA( iImage, (uint8*)pAvatarRGBA, AvatarLen*sizeof(uint8) );

				for (uint32 i = 0; i < (uAvatarWidth * uAvatarHeight * 4); i += 4)
				{
					uint8 Temp = pAvatarRGBA[i + 0];
					pAvatarRGBA[i + 0] = pAvatarRGBA[i + 2];
					pAvatarRGBA[i + 2] = Temp;
				}
			}
		}
		...
		if (NickName.Len() > 0 && CanAddUserToList(bIsOnline, bIsPlayingThisGame, bInASession))
		{
			...
			Friend->SetUserImage(pAvatarRGBA,uAvatarWidth,uAvatarHeight);
			Friend->Level = nLevel;
	...
	FriendsList.Friends.Sort([](TSharedRef<FOnlineFriendSteam> A,TSharedRef<FOnlineFriendSteam> B)
	{
		if(A->Presence.Status.State == EOnlinePresenceState::Online)
		{
			if(B->Presence.Status.State == EOnlinePresenceState::Online)
			{
				return A->GetUserId()->ToString() > B->GetUserId()->ToString();
			}
			return true;
		}
		if(B->Presence.Status.State == EOnlinePresenceState::Online)
